...
import type { CreatePotRequest, PotTransactionRequest, UpdatePotRequest } from '@/types/api';
...
export class PotService {
  ...
  static async updatePot(userId: string, potId: string, data: UpdatePotRequest): Promise<Pot> {
    await PotService.getPotById(userId, potId);
  ...
  static async deletePot(userId: string, potId: string): Promise<number> {
    const pot = await PotService.getPotById(userId, potId);
